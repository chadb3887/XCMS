...
class TMDB
{
	...
	{
		$movies = [];
		$result = $this->_call('search/movie', '&query=' . urlencode($movieTitle) . ($rYear ? '&primary_release_year=' . (int) $rYear : ''));
	...
	{
		$tvShows = [];
		$result = $this->_call('search/tv', '&query=' . urlencode($tvShowTitle) . ($rYear ? '&first_air_date_year=' . (int) $rYear : ''));
