...
class Release
{
	...
	{
		if (!in_array($attribute, ['source' => true, 'encoding' => true, 'resolution' => true, 'dub' => true])) {
	...
	private function parseType(&$title)
	{
		...
		$title = preg_replace_callback('#S(?<season>\\d{1,2})E(?<episode>\\d{1,2})#i', function($matches) use(&$type) {
			$type = self::TVSHOW;
			$this->setSeason((int) $matches[1]);

			if ($matches[2]) {
				$this->setEpisode((int) $matches[2]);
			}
