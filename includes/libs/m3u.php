...
class M3uParser
{
	...
	{
		return stripos($lineStr, '#EXTM3U') === 0;
	}
	...
	protected function isComment(string $lineStr): bool
	{
		...
		return !$matched && (strpos($lineStr, '#') === 0) && !$this->isExtM3u($lineStr);
	}
}
...
class ExtInf implements ExtTagInterface
{
	...
	static public function isMatch(string $lineStr): bool
	{
		return stripos($lineStr, '#EXTINF:') === 0;
	}
...
class ExtLogo implements ExtTagInterface
{
	...
	static public function isMatch(string $lineStr): bool
	{
		return stripos($lineStr, '#EXTLOGO:') === 0;
	}

	protected function makeData(string $lineStr): void
	{
		$tmp = substr($lineStr, 9);
		$logo = trim($tmp);
		$this->setLogo($logo);
	}
}
...
class ExtTv implements ExtTagInterface
{
	...
	static public function isMatch(string $lineStr): bool
	{
		return stripos($lineStr, '#EXTTV:') === 0;
	}
