...
function loadCLI()
{
	...
	if ($rData['action'] == 'certbot_generate') {
		if (file_exists(BIN_PATH . 'certbot/logs/xcms.log')) {
			unlink(BIN_PATH . 'certbot/logs/xcms.log');
		}
		...
		if (0 < count($rActiveDomains)) {
			foreach (['--dry-run ', ''] as $rDry) {
				if (XCMS::$rServers[SERVER_ID]['http_broadcast_port'] == 80) {
					$rCommand = 'sudo certbot ' . $rDry . '--config-dir ' . BIN_PATH . 'certbot/config --work-dir ' . BIN_PATH . 'certbot/work --logs-dir ' . BIN_PATH . 'certbot/logs certonly --agree-tos --expand --non-interactive --register-unsafely-without-email --webroot -w /home/xcms/www/';
				}
				else {
					...
					if (stripos(implode("\n", $rOutput), 'certificate and chain have been saved at') !== false) {
						...
						if ($rDirectory) {
							...
							if (file_exists($rCertificate) && file_exists($rChain) && file_exists($rPrivateKey)) {
								...
								shell_exec('chown xcms:xcms ' . BIN_PATH . 'nginx/conf/ssl.conf');
								$rInfo = XCMS::getCertificateInfo();
				...
				foreach (scandir(BIN_PATH . 'certbot/config/live/') as $rDir) {
					if (($rDir != '.') && ($rDir != '..')) {
						...
						if (in_array(strtolower($rDomain), array_map('strtolower', $rActiveDomains))) {
							$rInfo = XCMS::getCertificateInfo(BIN_PATH . 'certbot/config/live/' . $rDir . '/fullchain.pem');
		...
		shell_exec('chown -R xcms:xcms ' . BIN_PATH . 'certbot/');
		file_put_contents(BIN_PATH . 'certbot/logs/xcms.log', json_encode($rReturn));

		if ($rResult) {
			shell_exec(XCMS_HOME . 'service reload');
		}
