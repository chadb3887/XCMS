...
if (isset(XCMS::$rRequest['id']) && (!($rUser = getRegisteredUser(XCMS::$rRequest['id'])) || (XCMS::$rRequest['id'] == $rUserInfo['id']))) {
	goHome();
}
...
if ($rPermissions['allow_change_password'] || !isset($rUser)) {
	...
	echo ' value="';
	echo isset($rUser) ? '' : ($rPermissions['allow_change_username'] ? generateString(10 < XCMS::$rSettings['pass_length'] ? XCMS::$rSettings['pass_length'] : 10) : '');
...
if (0 < count($rPermissions['all_reports'])) {
	...
	if (0 < count($rPermissions['direct_reports'])) {
		...
		foreach ($rPermissions['direct_reports'] as $rUserID) {
			...
			echo $rUserID;
			echo '"';
			if (isset($rUser['owner_id']) && ($rUserID == $rUser['owner_id'])) {
				echo ' selected';
			}
...
if (1 < count($rPermissions['subresellers'])) {
	...
	foreach (getMemberGroups() as $rGroup) {
		...
		if (isset($rUser)) {
			if ((int) $rUser['member_group_id'] == (int) $rGroup['group_id']) {
				echo 'selected ';
			}
		}

		echo 'value="';
		echo (int) $rGroup['group_id'];
		echo '">';
