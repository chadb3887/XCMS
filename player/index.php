...
XCMS::verifyLicense();
if (isset(XCMS::$rRequest['search']) && isset(XCMS::$rRequest['type'])) {
	if (in_array(XCMS::$rRequest['type'], ['live' => true, 'movies' => true, 'series' => true])) {
		header('Location: ' . XCMS::$rRequest['type'] . '.php?search=' . urlencode(XCMS::$rRequest['search']));
		exit();
	}
}

$rPopularNow = [];
$rPopular = XCMS::unserialize(file_get_contents(CONTENT_PATH . 'tmdb_popular'));
if ((0 < count($rPopular['movies'])) && (0 < count($rUserInfo['vod_ids']))) {
	if (XCMS::$rSettings['player_hide_incompatible']) {
		...
		$db->query('SELECT `id`, `stream_display_name`, `year`, `rating`, `movie_propeties` FROM `streams` LEFT JOIN `webplayer_data` ON `webplayer_data`.`stream_id` = `streams`.`id` WHERE `id` IN (' . implode(',', $rPopular['movies']) . ') AND `id` IN (' . implode(',', $rUserInfo['vod_ids']) . ') ORDER BY FIELD(id, ' . implode(',', $rPopular['movies']) . ') ASC LIMIT 50;');
	}

	$rStreams = $db->get_rows();

	foreach ($rStreams as $rStream) {
		$rProperties = json_decode(PLATFORM == 'xcms' ? $rStream['movie_properties'] : $rStream['movie_propeties'], true);
		$rPopularNow[] = ['type' => 'movie', 'id' => $rStream['id'], 'title' => $rStream['stream_display_name'], 'year' => $rStream['year'] ?: NULL, 'rating' => $rStream['rating'], 'cover' => XCMS::validateImage($rProperties['movie_image']) ?: '', 'backdrop' => XCMS::validateImage($rProperties['backdrop_path'][0]) ?: ''];
	}
}
if ((0 < count($rPopular['series'])) && (0 < count($rUserInfo['series_ids']))) {
	if (XCMS::$rSettings['player_hide_incompatible']) {
		...
		$db->query('SELECT `id`, `title`, `releaseDate`, `rating`, `cover`, `backdrop_path` FROM `series` WHERE `id` IN (' . implode(',', $rPopular['series']) . ') AND `id` IN (' . implode(',', $rUserInfo['series_ids']) . ') ORDER BY FIELD(id, ' . implode(',', $rPopular['series']) . ') ASC LIMIT 50;');
	}
	...
	foreach ($rStreams as $rStream) {
		...
		$rPopularNow[] = ['type' => 'episodes', 'id' => $rStream['id'], 'title' => $rStream['title'], 'year' => $rStream['year'] ?: (substr($rStream['releaseDate'], 0, 4) ?: NULL), 'rating' => $rStream['rating'], 'cover' => XCMS::validateImage($rStream['cover']) ?: '', 'backdrop' => XCMS::validateImage($rBackdrop[0]) ?: ''];
	}
}

shuffle($rPopularNow);
$rPopularNow = array_slice($rPopularNow, 0, 20);
$rMovies = XCMS::getUserStreams($rUserInfo, ['movie'], NULL, NULL, 'added', NULL, NULL, 0, 20);
$rSeries = XCMS::getUserSeries($rUserInfo, NULL, NULL, 'added', $rSearchBy, NULL, 0, 20);
$_TITLE = 'Home';
include 'header.php';
...
foreach ($rMovies['streams'] as $rStreamID => $rStream) {
	...
	echo urlencode(XCMS::validateImage($rProperties['movie_image']) ?: '');
	echo '&w=267&h=400" alt="">' . "\r\n" . '                                        <a href="movie.php?id=';
	echo $rStream['id'];
	...
	echo '</a></h3>' . "\r\n" . '                                        <span class="card__rate">';
	echo $rStream['year'] ? (int) $rStream['year'] . ' &nbsp; ' : '';
	echo '<i class="icon ion-ios-star"></i>';
...
foreach ($rSeries['streams'] as $rStreamID => $rStream) {
	echo '                            <div class="col-6 col-sm-4 col-lg-3 col-xl-3">' . "\r\n" . '                                <div class="card">' . "\r\n" . '                                    <div class="card__cover">' . "\r\n" . '                                        <img loading="lazy" src="resize.php?url=';
	echo urlencode(XCMS::validateImage($rStream['cover']) ?: '');
	...
	echo '</a></h3>' . "\r\n" . '                                        <span class="card__rate">';
	echo $rStream['year'] ? (int) $rStream['year'] . ' &nbsp; ' : ($rStream['releaseDate'] ? (int) substr($rStream['releaseDate'], 0, 4) . ' &nbsp; ' : '');
	echo '<i class="icon ion-ios-star"></i>';
