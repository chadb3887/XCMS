...
if ($_PAGE == 'profile') {
	...
	if (0 < count($rStreamIDs)) {
		...
		if ($rFilterBy == 'epg') {
			echo '    window.XCMS.Listings.HideEmpty = 1;' . "\r\n" . '    ';
		}
		else {
			echo '    window.XCMS.Listings.HideEmpty = 0;' . "\r\n" . '    ';
		}
	}
	...
	foreach (XCMS::getOrderedCategories($rUserInfo['category_ids'], $_PAGE) as $rCategory) {
...
else if (($_PAGE == 'movie') || ($_PAGE == 'episodes')) {
	...
	if (isset(XCMS::$rRequest['season'])) {
		echo '        $(\'html,body\').animate({' . "\r\n" . '            scrollTop: $(".seasons").offset().top - 90' . "\r\n" . '        });' . "\r\n" . '        ';
	}

	echo '    });' . "\r\n" . '    ';
}
else if (($_PAGE == 'movies') || ($_PAGE == 'series')) {
	echo '    $(document).ready(function () {' . "\r\n" . '        $(".filter__btn").click(function() {' . "\r\n" . '            var rSort = JSON.parse(\'';
	echo str_replace('\'', '\\\'', json_encode(array_flip($rSortArray)));
	echo '\');' . "\r\n" . '            ';
	$rCategories = [];

	foreach (XCMS::getOrderedCategories($rUserInfo['category_ids'], $_PAGE == 'movies' ? 'movie' : 'series') as $rCategory) {
		$rCategories[$rCategory['title']] = $rCategory['id'];
	}
