...
echo XCMS::$rSettings['server_name'] ?: 'XCMS Web Player';
echo ' - ';
echo $_TITLE;
echo '</title>' . "\r\n" . '    <style>' . "\r\n" . '    .seasons__cover {' . "\r\n" . '        filter: blur(';
echo (int) XCMS::$rSettings['player_blur'] ?: 0;
echo 'px) !important;' . "\r\n" . '        opacity: ';
echo (int) XCMS::$rSettings['player_opacity'] ?: 10;
echo '%;' . "\r\n" . '    }' . "\r\n" . '    .details__bg {' . "\r\n" . '        filter: blur(';
echo (int) XCMS::$rSettings['player_blur'] ?: 0;
echo 'px) !important;' . "\r\n" . '        opacity: ';
echo (int) XCMS::$rSettings['player_opacity'] ?: 10;
...
echo '">Home</a>' . "\r\n\t\t\t\t\t\t\t\t" . '</li>' . "\r\n" . '                                ';
if ((PLATFORM != 'xcms') || (in_array(1, $rUserInfo['allowed_outputs']) && !XCMS::$rSettings['disable_hls'])) {
...
if (in_array($_PAGE, ['movies' => true, 'movie' => true])) {
	echo ' selected';
}

echo '>Movies</option>' . "\r\n" . '                                <option value="series"';

if (in_array($_PAGE, ['series' => true, 'episodes' => true])) {
	echo ' selected';
}
