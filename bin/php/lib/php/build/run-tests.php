...
function find_files($dir, $is_ext_dir = false, $ignore = false)
{
	...
	while (($name = readdir($o)) !== false) {
		if (is_dir($dir . '/' . $name) && !in_array($name, ['.' => true, '..' => true, '.svn' => true])) {
...
function run_test($php, $file, $env)
{
	...
	if ($temp_source && $temp_target) {
		...
		if (!is_dir(dirname($copy_file))) {
			mkdir(dirname($copy_file), 511, true) || error('Cannot create output directory - ' . dirname($copy_file));
		}
	...
	if (array_key_exists('EXTENSIONS', $section_text)) {
		...
		$ext_dir = shell_exec($php . ' ' . $pass_options . ' ' . $ext_params . ' -d display_errors=0 -r "echo ini_get(\'extension_dir\');"');
		$extensions = preg_split('/[' . "\n\r" . ']+/', trim($section_text['EXTENSIONS']));
		$loaded = explode(',', shell_exec($php . ' ' . $pass_options . ' ' . $ext_params . ' -d display_errors=0 -r "echo implode(\',\', get_loaded_extensions());"'));
	...
	if ($leak_check) {
		...
		else if (version_compare($valgrind_version, '3.3.0', '>=')) {
	...
	if ($slow_min_ms <= $time * 1000) {
		$PHP_FAILED_TESTS['SLOW'][] = ['name' => $file, 'test_name' => (is_array($IN_REDIRECT) ? $IN_REDIRECT['via'] : '') . $tested . (' [' . $tested_file . ']'), 'output' => '', 'diff' => '', 'info' => $time];
	}
...
function compute_summary()
{
	...
	$sum_results = ['PASSED' => 0, 'WARNED' => 0, 'SKIPPED' => 0, 'FAILED' => 0, 'BORKED' => 0, 'LEAKED' => 0, 'XFAILED' => 0];
	...
	foreach ($sum_results as $v => $n) {
		$percent_results[$v] = ($n * 100.0) / $n_total;
	}
}
...
function get_summary($show_ext_summary, $show_html)
{
	...
	if ($x_total) {
		$x_warned = ($sum_results['WARNED'] * 100.0) / $x_total;
		$x_failed = ($sum_results['FAILED'] * 100.0) / $x_total;
		$x_xfailed = ($sum_results['XFAILED'] * 100.0) / $x_total;
		$x_leaked = ($sum_results['LEAKED'] * 100.0) / $x_total;
		$x_passed = ($sum_results['PASSED'] * 100.0) / $x_total;
	}
	else {
...
function junit_mark_test_as($type, $file_name, $test_name, $time = NULL, $message = '', $details = '')
{
	...
	$time = ($time !== NULL ? $time : junit_get_timer($file_name));
...
function junit_suite_record($suite, $param, $value = 1)
{
	global $JUNIT;
	$JUNIT[$suite] += $value;
	$JUNIT['suites'][$suite][$suite] += $value;
}
...
if (isset($argc) && (1 < $argc)) {
	...
	if ($test_cnt) {
		...
		junit_save_xml();
		if ((getenv('REPORT_EXIT_STATUS') !== '0') && (getenv('REPORT_EXIT_STATUS') !== 'no') && ($sum_results['FAILED'] || $sum_results['BORKED'])) {
			exit(1);
		}
