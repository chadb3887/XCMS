...
function panelLog($rType, $rMessage, $rExtra = '', $rFile, $rLine = 0)
{
	$rData = ['type' => $rType, 'message' => $rMessage, 'extra' => $rExtra, 'line' => $rLine, 'file' => $rFile, 'time' => time()];
...
function generateError($rError, $rKill = true)
{
	...
	if ($rSettings['debug_show_errors']) {
		...
		echo '<html><head><title>XCMS - Debug Mode</title><link href="https://fonts.googleapis.com/css?family=Montserrat:200,400,700" rel="stylesheet"><style>' . $rStyle . '</style></head><body><div id="notfound"><div class="notfound"><div class="notfound-404"><h1>XCMS.ONE</h1><h2>' . $rError . '</h2><br/></div><p>' . $rErrorDescription . '</p></div></div></body></html>';
...
{
	panellog('exception', $e->getMessage(), $e->getTraceAsString(), $e->getFile(), $e->getLine());
}
...
define('XCMS_VERSION', '1.0.1');
define('XCMS_REVISION', '0');
...
define('CRON_PATH', XCMS_HOME . 'crons/');
define('SIGNALS_PATH', XCMS_HOME . 'signals/');
...
define('FFMPEG_BIN_50', BIN_PATH . 'ffmpeg_bin/5.0/ffmpeg');
define('FFPROBE_BIN_40', BIN_PATH . 'ffmpeg_bin/4.0/ffprobe');
define('FFPROBE_BIN_50', BIN_PATH . 'ffmpeg_bin/5.0/ffprobe');
$rShowErrors = false;
...
if (!@$argc) {
	...
	if (file_exists(CACHE_TMP_PATH . 'settings')) {
		...
		if (is_array($rSettings) && file_exists(CACHE_TMP_PATH . 'allowed_domains') && $rSettings['verify_host']) {
			...
			if (is_array($rAllowedDomains) && !in_array(HOST, $rAllowedDomains) && (HOST != 'xcms') && !filter_var(HOST, FILTER_VALIDATE_IP)) {
