...
{
	$rData = ['type' => $rType, 'message' => $rMessage, 'extra' => $rExtra, 'line' => $rLine, 'time' => time()];
...
if (!@$argc) {
	...
	if (is_array($rSettings) && $rSettings['verify_host']) {
		...
		if (is_array($rAllowedDomains) && (0 < count($rAllowedDomains)) && !in_array(HOST, $rAllowedDomains) && (HOST != 'xcms') && !filter_var(HOST, FILTER_VALIDATE_IP)) {
